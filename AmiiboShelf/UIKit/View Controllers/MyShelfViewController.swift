...
class MyShelfViewController: AmiiboViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        let shelfImage = #imageLiteral(resourceName: "Shelf-Image")
        shelfView.backgroundColor = UIColor(
            patternImage: shelfImage.resize(width: Int(shelfCollectionView.frame.width),
                                            height: Int(shelfCollectionView.frame.height/4.8)) ?? UIImage()
        )
...
//        Amiibo.from(cdAmiibos: CoreDataManager.current.fetchAllShelfAmiibo())
    ...
    func showNoAmiiboOnShelfAlert() {
        let alertController = UIAlertController(title: "My Shelf", message: """
            There's no amiibo on the shelves! Add one by selecting an Amiibo -> Add to Shelf
...
extension MyShelfViewController: UICollectionViewDelegate,
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        guard let cell = shelfCollectionView
            .dequeueReusableCell(withReuseIdentifier: "ShelfItem", for: indexPath)
                as? ShelfItemCollectionViewCell else {
...
#Preview {
    var controller = MyShelfViewController()
    return controller
}
